        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @if ($orders->isNotEmpty())
                    <a href="{{ route('orders.export', ['status' => $filter, 'q' => $q]) }}" class="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-[var(--radius-md)] border border-border px-3 text-sm font-medium text-ink-soft hover:bg-muted-surface"><x-lucide-download class="size-4" /> CSV</a>
                @endif
            </div>
        </div>
                ...
                <div class="overflow-x-auto scroll-thin">
                    <table class="w-full text-sm">
                        <thead><tr class="border-b border-border bg-surface text-left text-xs text-muted">
                            ...
                            <th class="px-4 py-3 text-right font-medium">Jumlah</th><th class="px-4 py-3 font-medium">Status</th><th class="px-4 py-3 font-medium">Tarikh</th><th class="px-4 py-3"></th>
                        </tr></thead>
                        <tbody class="divide-y divide-border">
                            @foreach ($orders as $o)
                                <tr class="transition-colors hover:bg-muted-surface/50">
                                    <td class="px-4 py-3"><a href="{{ route('orders.show', $o) }}" class="block"><div class="font-medium text-ink hover:text-primary">{{ $o->customer }}</div><div class="text-xs text-muted">{{ $o->phone }}</div></a></td>
                                    ...
                                    <td class="px-4 py-3 text-right"><a href="{{ route('orders.show', $o) }}" class="inline-flex text-muted hover:text-primary"><x-lucide-chevron-right class="size-4" /></a></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
